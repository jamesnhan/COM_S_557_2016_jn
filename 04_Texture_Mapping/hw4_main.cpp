...
void createSquare(void)
{
	...
	vertices[i++] = -0.5; vertices[i++] = -0.5; vertices[i++] = 0.5; // Bottom left corner
	colors[0] = 1.0; colors[1] = 1.0; colors[2] = 1.0; // Bottom left corner
	vertices[i++] = 0.0; vertices[i++] = 0.0; // UV bottom left corner;

	vertices[i++] = -0.5; vertices[i++] = 0.5; vertices[i++] = 0.5; // Top left corner
	colors[3] = 1.0; colors[4] = 0.0; colors[5] = 0.0; // Top left corner
	vertices[i++] = 0.0; vertices[i++] = 1.0; // UV top left corner;

	vertices[i++] = 0.5; vertices[i++] = 0.5; vertices[i++] = 0.5; // Top Right corner
	colors[6] = 0.0; colors[7] = 1.0; colors[8] = 0.0; // Top Right corner
	vertices[i++] = 1.0; vertices[i++] = 1.0; // UV top right corner;

	vertices[i++] = 0.5; vertices[i++] = -0.5; vertices[i++] = 0.5; // Bottom right corner
	colors[9] = 0.0; colors[10] = 0.0; colors[11] = 1.0; // Bottom right corner
	vertices[i++] = 1.0; vertices[i++] = 0.0; // UV bottom right corner;
	...
	int tex_idx = glGetAttribLocation(program, "in_TexCoord");

	glVertexAttribPointer((GLuint)tex_idx, 2, GL_FLOAT, GL_TRUE, 5 * sizeof(GLfloat), (const GLvoid*)(3 * sizeof(GLfloat)));
	glEnableVertexAttribArray(tex_idx); // Disable our Vertex Array Object
	...
	delete[] colors; // Delete our colors from memory
	...
	-----------------------------------------------------------------------------------*/
	unsigned char* a = ImgLoader::Load("a.bmp");
	//unsigned char* b = ImgLoader::Load("b.bmp");
	//unsigned char* c = ImgLoader::Load("c.bmp");
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 512, 512, 0, GL_BGR, GL_UNSIGNED_BYTE, a);
	...
	if (params == GL_FALSE)
	{
		cerr << "Program " << program << " has not been linked. Textures cannot be added." << endl;
		return;
	}
	...
	if (idx == -1)
	{
		cerr << "Uniform variable tex was not added to the shader program." << endl;
		return;
	}
...
int main(int argc, const char * argv[])
{
    ...
    glCompileShader(fs);
	bool ret = CheckShaderLocal(fs, GL_FRAGMENT_SHADER);
